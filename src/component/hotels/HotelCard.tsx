      ...
      <Card>
        ...
        <div className="flex gap-0.5">
          {Array.from({ length: fullStars }).map((_, idx) => (
            <img key={idx} src={star} alt="별" />
          ))}
          {hasHalfStar ? <img src={halfStar}/> : null}
        </div>
        <Text size='text-sm' color='sub'>{hotel.review_count}개의 리뷰</Text>
        <Text size='text-lg' align='right' color='primary'>{formatNumberWithComma(hotel.cheapest_price)}원</Text>
      </Card>
    </div>
  );
};
